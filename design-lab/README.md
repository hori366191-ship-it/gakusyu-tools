...
- **最終移植**: `design-lab/themes/pop.css` が完成したら `site/themes/pop.css` へ手動コピーし、ポータルに切替UIを追加する（Phase 4）
...
- **Pop — crayon-coded terminal on cream paper**（Refero Styles `2bd7363d-7aae-4b1f-9d5a-1edeb17ca567`）
- トークンは `tokens/pop.css` に全量保存。出典は `Inspired by Pop via Refero Styles` と明記

## 編集ルール

1. `site/` を直接編集しない。必ず `design-lab/` で試す
2. 画像は自作SVGを `assets/` に置く（Popのアヒルを丸コピしない）
...
- まずは `標準` テーマで動作させ、`design-lab/app10/` 等で試作してから `site/themes/pop.css` 等へ追記して移植する
...
- Phase 4: `site/themes/pop.css` へ移植し、ポータルに切替UIを追加（`localStorage` で `portal→app` 伝播）
